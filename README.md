...
### 🆕 v3.4.2の改善点（エラーハンドリング強化）
- ✅ **保存前検証追加**: メーター値の整合性を保存前にチェック
  - 前のタンクより小さい値があれば警告表示
  - 例: 「⚠️ 入力エラー\n\n・牛舎2: 50L（前のタンク: 100L）」
  - 保存処理を中止して修正を促す
- ✅ 複数のエラーを一括表示
...
### 🆕 v3.4.1の改善点（バグ修正＆改善）
- 🐛 **バグ修正1**: 1文字入力で警告が出る問題を修正
  - 入力中は警告を表示せず、計算結果のみ更新
- 🐛 **バグ修正2**: タンクをスキップした場合の計算ミスを修正
  - 例: タンク1(100L) → タンク2(空欄) → タンク3(300L) = 200Lと正しく計算
  - 前の入力済みタンクを自動で探して差分計算
- ✅ **改善**: CSV出力後の完了メッセージを削除
  - ダウンロードが成功すれば視覚的に確認できるため
  - 余計なクリック操作を削減
